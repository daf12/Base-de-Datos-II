...
create or replace function SUMA_EDADES3(SEXOEST VARCHAR(30))  RETURNS int
    BEGIN
...
SELECT nombres,apellidos
    FROM estudiantes AS ES
WHERE SUMA_EDADES3('masculino') mod 2=1;
...
CREATE OR REPLACE FUNCTION CONTENADO(par1 VARCHAR(25), par2 VARCHAR(25))
    RETURNS VARCHAR(50)
BEGIN
    DECLARE concatenado VARCHAR(50) DEFAULT '';

    SET concatenado = CONCAT ('NOMBRE:',' ', par1,'  ', 'APELLIDO:',' ', par2);

    RETURN concatenado;

END;

SELECT CONTENADO(ES.nombres,ES.apellidos) AS DATOS_PERSONALES, CONTENADO2(ES.gestion,ES.edad) AS DATOS_PERSONALES2
FROM estudiantes AS ES;


CREATE OR REPLACE FUNCTION CONTENADO2(par1 VARCHAR(25), par2 int)
    RETURNS VARCHAR(50)
BEGIN
    DECLARE concatenado VARCHAR(50) DEFAULT '';

    SET concatenado = CONCAT('GESTION:','  ',par1,' ', 'EDAD:',' ',par2);

    RETURN concatenado;

END;

SELECT CONTENADO2(ES.gestion,ES.edad)
FROM estudiantes AS ES
...
-- ejercicio 1
    CREATE OR REPLACE FUNCTION CONTENADO3(par1 int , par2 int,par3 VARCHAR(25),par4 VARCHAR(25))
    RETURNS VARCHAR(150)
BEGIN
    DECLARE concatenado VARCHAR(150) DEFAULT '';

    SET concatenado = CONCAT('GESTION:','  ',par1,' ', 'EDAD:',' ',par2,' NOMBRES: ',' ', par3 ,' APELLIDOS: ',' ', par4) ;
...
SELECT CONTENADO3(gestion,edad,nombres,apellidos)
    FROM estudiantes AS ES

-- EJERCICIO 2

select e.nombres, e.apellidos, i.semestre
from estudiantes e
...
from estudiantes e
inner join inscripcion i on e.id_est = i.id_est
WHERE I.gestion = 2016 or 2017;


create or replace function agrF1() returns int
begin
    return
        (select )
end;
